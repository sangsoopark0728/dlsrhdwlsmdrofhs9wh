...
class StudyTimerApp(tk.Tk):
    ...
    def _rebuild_sidebar_add(self):
        ...
        for var in [self.character, self.prop]:
    ...
    def _load_weekly_day_to_today(self):
        ...
        if not day_subjects:
            ...
            return
        self.subjects = []
        for i, src in enumerate(day_subjects):
            subj = dict(src)
            subj["color"] = SUBJECT_COLORS[i % len(SUBJECT_COLORS)]
            subj["elapsed_secs"] = 0.0
            subj["completed"] = False
            subj.setdefault("character", "turtle")
            subj.setdefault("prop", "box")
