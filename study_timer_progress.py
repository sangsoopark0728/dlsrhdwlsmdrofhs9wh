        ...
        if system == "Windows":
            ...
            if onedrive:
                for n in ["바탕 화면", "Desktop"]:
                    candidate = os.path.join(onedrive, n)
                    if os.path.isdir(candidate):
                        return candidate
        elif system == "Darwin":
...
# ════════════════════════════════════════════════════════
#  공통 다이얼로그 (스터디 타이머 디자인)
# ════════════════════════════════════════════════════════
class StyledDialog(tk.Toplevel):
    """스터디 타이머 디자인의 커스텀 다이얼로그"""
    def __init__(self, parent, title, message, buttons, icon="❓"):
        ...
        tk.Label(card, text=icon, font=("Segoe UI Emoji", 36),
                 bg=CARD_BG, fg=ACCENT).pack(pady=(20, 6))

        tk.Label(card, text=title, font=("Georgia", 14, "bold"),
                 bg=CARD_BG, fg=TEXT).pack()

        tk.Label(card, text=message, font=("Malgun Gothic", 11),
                 bg=CARD_BG, fg=TEXT_DIM, justify="center",
                 wraplength=400).pack(pady=(10, 18), padx=20)

        btn_frame = tk.Frame(card, bg=CARD_BG)
        btn_frame.pack(pady=(0, 20))
        for label, color, value in buttons:
            b = tk.Button(btn_frame, text=label, font=("Malgun Gothic", 11, "bold"),
                          bg=color, fg="#000000",
...
class StudyTimerApp(tk.Tk):
    ...
    def _build_sidebar(self):
        ...
        # ── 현재 선택된 캐릭터/소품 미리보기 라벨
        self.preview_label = tk.Label(add_frame, text="",
                                      font=("Malgun Gothic", 9),
                                      bg=SIDEBAR_BG, fg=TEXT_DIM, justify="left")
        self.preview_label.pack(anchor="w", pady=(0, 6))
        self._update_preview_label()

        # 캐릭터/소품 변경 시 미리보기 자동 갱신
        self.character.trace_add("write", lambda *_: self._update_preview_label())
        self.prop.trace_add("write", lambda *_: self._update_preview_label())
    ...
    def _update_preview_label(self):
        """사이드바 '+ 과목 추가' 바로 위에 현재 선택된 캐릭터·소품을 표시"""
        char = self.character.get()
        prop_ = self.prop.get()
        c_icon = CHARACTER_ICONS.get(char, "")
        p_icon = PROP_ICONS.get(prop_, "")
        self.preview_label.configure(
            text=f"추가 시 포함:  {c_icon} {CHARACTER_LABELS.get(char, char)}  +  {p_icon} {PROP_LABELS.get(prop_, prop_)}"
        )
    ...
    def _build_schedule_view(self):
        ...
        # ── 하단 오른쪽: 타이머 시작 버튼
        bottom_bar = tk.Frame(self.schedule_view, bg=BG)
        bottom_bar.pack(fill="x", padx=32, pady=(0, 24))

        self.start_btn = tk.Button(
            bottom_bar,
            text="▶  타이머 시작",
            font=("Malgun Gothic", 15, "bold"),
            bg=LIGHT_RED, fg="#000000",
            activebackground=LIGHT_RED_HOVER,
            relief="flat", cursor="hand2",
            padx=32, pady=16, bd=0,
    ...
    def _build_timer_view(self):
        ...
        tk.Button(top_bar, text="🏠  메인화면",
                  ...
                  command=self._go_main_from_timer).pack(side="left")
        ...
        # ── 타이머 화면의 캐릭터/소품 표시 라벨
        self.lbl_char_prop = tk.Label(top, text="", font=("Malgun Gothic", 13),
                                      bg=BG, fg=TEXT_DIM)
    ...
    def _refresh_subject_list(self):
        ...
        for i, subj in enumerate(self.subjects):
            ...
            elapsed_secs = float(subj.get("elapsed_secs", 0.0))
            total_secs = max(1, subj["minutes"] * 60)
            progress = 1.0 if subj.get("completed", False) else min(elapsed_secs / total_secs, 1.0)
            status_text = "완료" if subj.get("completed", False) else f"{int(progress * 100)}%"

            tk.Label(row2, text=f"  진행도 {status_text}",
                     font=("Malgun Gothic", 11),
                     bg=CARD_BG,
                     fg=SUCCESS if subj.get("completed", False) else TEXT_DIM).pack(side="left")
            ...
            c_icon    = CHARACTER_ICONS.get(char_key, "🐢")
            p_icon    = PROP_ICONS.get(prop_key, "📦")
            c_label   = CHARACTER_LABELS.get(char_key, char_key)
            p_label   = PROP_LABELS.get(prop_key, prop_key)

            badge_char = tk.Label(row2,
                                  text=f"{c_icon} {c_label}",
                                  font=("Malgun Gothic", 10),
                                  bg=self._darken_hex(subj["color"], 0.65),
                                  fg=subj["color"],
                                  padx=8, pady=2,
                                  relief="flat")
            badge_char.pack(side="left", padx=(0, 4))

            badge_prop = tk.Label(row2,
                                  text=f"{p_icon} {p_label}",
                                  font=("Malgun Gothic", 10),
                                  bg=self._darken_hex(subj["color"], 0.65),
                                  fg=subj["color"],
                                  padx=8, pady=2,
                                  relief="flat")
            badge_prop.pack(side="left")
    ...
    def _load_subject(self, idx):
        ...
        char_key = subj.get("character", "turtle")
        prop_key = subj.get("prop", "box")
        self.character.set(char_key)
        self.prop.set(prop_key)
        ...
        c_label = CHARACTER_LABELS.get(char_key, char_key)
        p_label = PROP_LABELS.get(prop_key, prop_key)
        self.lbl_char_prop.configure(
            text=f"{c_icon} {c_label}  +  {p_icon} {p_label}",
            fg=color
        )

        self.anim_progress = self._elapsed_at_pause / self.total_secs if self.total_secs > 0 else 0.0
    ...
    def _save_current_progress(self):
        if not self.subjects or self.current_subject >= len(self.subjects):
            return
        subj = self.subjects[self.current_subject]
        total_secs = subj["minutes"] * 60
        elapsed = min(self._elapsed(), total_secs)
        subj["elapsed_secs"] = elapsed
        subj["completed"] = elapsed >= total_secs
    ...
    def _go_main_from_timer(self):
        ...
        if dlg.result == "yes":
            ...
            if self.timer_running and not was_paused:
                self._start_mono = time.monotonic()
                self.timer_paused = False
                self.pause_btn.configure(text="⏸  일시정지", bg=WARNING)
                self._render_loop()
    ...
    def _show_all_done_window(self):
        ...
        accent_bar = tk.Frame(card, bg=SUCCESS, width=6)
        accent_bar.pack(side="left", fill="y")

        body = tk.Frame(card, bg=CARD_BG)
        body.pack(side="left", fill="both", expand=True, padx=24, pady=24)

        tk.Label(body, text="🎉", font=("Segoe UI Emoji", 56),
                 bg=CARD_BG, fg=SUCCESS).pack(pady=(10, 4))

        tk.Label(body, text="축하합니다!", font=("Georgia", 22, "bold"),
                 bg=CARD_BG, fg=SUCCESS).pack(pady=(0, 4))

        tk.Label(body, text="오늘의 공부량을 끝내셨습니다.",
                 font=("Malgun Gothic", 13),
                 bg=CARD_BG, fg=TEXT).pack(pady=(4, 6))

        tk.Label(body, text="📚  수고하셨어요! 푹 쉬세요  ☕",
                 font=("Malgun Gothic", 10),
                 bg=CARD_BG, fg=TEXT_DIM).pack(pady=(0, 18))
    ...
    def _reset_all(self):
        ...
        self.schedule_view.pack(fill="both", expand=True)
        self.pause_btn.configure(text="⏸  일시정지", bg=WARNING, state="normal")
        self.lbl_subject_name.configure(text="", fg=ACCENT)
        self.lbl_subject_index.configure(text="")
        self.lbl_timer.configure(text="00:00", fg=TEXT)
        self.lbl_char_prop.configure(text="")
    ...
    def _refresh_mini_schedule(self, current_idx, color):
        ...
        for i, subj in enumerate(self.subjects):
            if i < current_idx:
                ...
                sc, st, ab = BORDER, "○", BG
            char_icon = CHARACTER_ICONS.get(subj.get("character", "turtle"), "")
            prop_icon = PROP_ICONS.get(subj.get("prop", "box"), "")
            ...
            tk.Label(pill,
                     text=f"{st} {subj['name']} ({subj['minutes']}분)  {char_icon}{prop_icon}",
    ...
    def _draw_scene(self, progress):
        ...
        if char == "turtle":
            ...
            self._draw_dinosaur(c, char_x, gy, t, color)
        elif char == "dog":
    ...
    def _draw_box(self, c, px, gy, progress, t, color):
        ...
        c.create_polygon(bx + 14, by, bx + 20, by - 6,
                          bx + 20, by + 22 - 6, bx + 14, by + 22,
                          fill=self._darken(color, 0.55), outline="", tags="dyn")
        c.create_polygon(bx - 14, by, bx + 14, by,
                          bx + 20, by - 6, bx - 8,  by - 6,
    ...
    def _draw_turtle(self, c, cx, gy, t, color):
        ...
        for lx, phase in [(-14, 0), (-6, math.pi)]:
            ...
            lby   = cy + 12 + swing
            c.create_oval(cx + lx - 7, lby - 5, cx + lx + 7, lby + 5,
                          fill="#3d8b65", outline="#1b4332", width=1, tags="dyn")
            c.create_oval(cx + lx - 5, lby + 1, cx + lx + 5, lby + 8,
        ...
        for lx, phase in [(8, math.pi), (16, 0)]:
            swing = math.sin(walk + phase) * 6
            lby   = cy + 10 + swing
            c.create_oval(cx + lx - 6, lby - 5, cx + lx + 6, lby + 5,
                          fill="#3d8b65", outline="#1b4332", width=1, tags="dyn")
            c.create_oval(cx + lx - 5, lby + 1, cx + lx + 5, lby + 8,
                          fill="#2e6b4f", outline="#1b4332", width=1, tags="dyn")
    ...
    def _draw_rabbit(self, c, cx, gy, t, color):
        if self.timer_paused:
            walk = 0.0; bob = 0.0; ear_sway = 0.0
        else:
            walk     = t * 5.0
            bob      = math.sin(t * 10) * 2.5
            ear_sway = math.sin(t * 4) * 3
        ...
        c.create_oval(cx - 24, cy + 4, cx - 15, cy + 14,
                      fill="#f0eaec", outline="", tags="dyn")

        for lx, phase in [(-12, 0), (0, math.pi)]:
            swing = math.sin(walk + phase) * 7
            lby   = cy + 14 + swing
            c.create_oval(cx + lx - 10, lby - 4, cx + lx + 10, lby + 6,
        ...
        for lx, phase in [(10, math.pi), (18, 0)]:
            ...
            lby   = cy + 10 + swing
            c.create_oval(cx + lx - 7, lby - 4, cx + lx + 7, lby + 5,
                          fill="#f0e6eb", outline="#c9b2c5", width=1, tags="dyn")
    ...
    def _draw_dinosaur(self, c, cx, gy, t, color):
        ...
        c.create_polygon(
            ...
            cx - 16, cy + 8,
            cx - 16, cy + 14,
            fill=body_color, outline=body_dark, width=1, tags="dyn"
        )

        for lx, phase in [(-8, 0), (2, math.pi)]:
            swing = math.sin(walk + phase) * 6
            lby   = cy + 16 + swing
            c.create_oval(cx + lx - 8, cy + 4,
                          cx + lx + 8, cy + 18,
                          fill=body_color, outline=body_dark, width=1, tags="dyn")
            c.create_oval(cx + lx - 9, lby + 2,
                          cx + lx + 11, lby + 9,
                          fill=body_dark, outline=body_dark, width=1, tags="dyn")
            for toe in [-6, 0, 6]:
                c.create_polygon(cx + lx + toe, lby + 6,
                                 cx + lx + toe + 2, lby + 10,
                                 cx + lx + toe - 2, lby + 10,
                                 fill="#1a1a1a", outline="", tags="dyn")

        c.create_oval(cx - 18, cy - 12, cx + 22, cy + 18,
                      fill=body_color, outline=body_dark, width=2, tags="dyn")
        c.create_oval(cx - 10, cy + 0, cx + 18, cy + 16,
                      fill=belly_color, outline="", tags="dyn")
        for ly in [cy + 4, cy + 9]:
            c.create_line(cx - 6, ly, cx + 14, ly,
                          fill=self._darken(belly_color, 0.2), width=1, tags="dyn")

        spike_pts = [
            (cx - 16, cy - 8), (cx - 12, cy - 16),
            (cx - 6,  cy - 12), (cx - 2, cy - 20),
            (cx + 4,  cy - 14), (cx + 10, cy - 22),
            (cx + 16, cy - 16), (cx + 22, cy - 22),
        ]
        for i in range(0, len(spike_pts) - 1, 2):
            p1 = spike_pts[i]; p2 = spike_pts[i+1]
            mid_x = (p1[0] + p2[0]) / 2 + 4
            c.create_polygon(p1[0], p1[1], p2[0], p2[1],
                             mid_x, p1[1] - 2,
                             fill=color, outline=self._darken(color, 0.4),
                             width=1, tags="dyn")
        ...
        c.create_oval(hx - 12, hy - 10, hx + 18, hy + 12,
                      fill=body_color, outline=body_dark, width=2, tags="dyn")

        c.create_arc(hx - 6, hy + 0, hx + 18, hy + 14,
                     start=180, extent=180,
                     fill=self._darken(body_color, 0.5),
                     outline=body_dark, width=1, tags="dyn")
        for tx in [hx - 2, hx + 4, hx + 10]:
            c.create_polygon(tx, hy + 7, tx + 3, hy + 12, tx - 3, hy + 12,
                             fill="white", outline="", tags="dyn")

        ex, ey = hx + 6, hy - 3
        c.create_oval(ex - 5, ey - 5, ex + 5, ey + 5,
                      fill="white", outline=body_dark, width=1, tags="dyn")
        c.create_oval(ex - 2, ey - 2, ex + 3, ey + 3,
                      fill="#1a1a1a", tags="dyn")
        c.create_oval(ex - 1, ey - 4, ex + 1, ey - 2,
                      fill="white", tags="dyn")

        c.create_oval(hx + 14, hy + 2, hx + 16, hy + 4,
                      fill=body_dark, outline="", tags="dyn")
    ...
    def _draw_dog(self, c, cx, gy, t, color):
        if self.timer_paused:
            walk = 0.0; bob = 0.0; tail_swing = 0.0
        else:
            walk = t * 5.0
            bob  = math.sin(t * 10) * 2.0
            tail_swing = math.sin(t * 12) * 6
        ...
        tail_x_end = cx - 24 + tail_swing
        tail_y_end = cy - 14
        c.create_line(cx - 16, cy - 4,
                      cx - 22, cy - 10,
                      tail_x_end, tail_y_end,
                      fill=body_color, width=6,
                      capstyle="round", tags="dyn")
        c.create_oval(tail_x_end - 4, tail_y_end - 4,
                      tail_x_end + 4, tail_y_end + 4,
                      fill=body_color, outline=body_dark, width=1, tags="dyn")

        for lx, phase in [(-10, 0), (-2, math.pi)]:
            swing = math.sin(walk + phase) * 5
            lby   = cy + 14 + swing
            c.create_oval(cx + lx - 5, cy + 4,
                          cx + lx + 5, cy + 18,
                          fill=body_color, outline=body_dark, width=1, tags="dyn")
            c.create_oval(cx + lx - 6, lby + 1,
                          cx + lx + 7, lby + 7,
                          fill=body_dark, outline=body_dark, width=1, tags="dyn")

        c.create_oval(cx - 18, cy - 8, cx + 18, cy + 16,
                      fill=body_color, outline=body_dark, width=2, tags="dyn")
        c.create_oval(cx - 10, cy + 2, cx + 14, cy + 14,
                      fill=belly, outline="", tags="dyn")

        c.create_oval(cx - 8, cy - 4, cx - 2, cy + 1,
                      fill=color, outline="", tags="dyn")
        c.create_oval(cx + 2, cy - 6, cx + 10, cy + 0,
                      fill=color, outline="", tags="dyn")
        ...
        hx, hy = cx + 20, cy - 14
        c.create_oval(hx - 13, hy - 11, hx + 14, hy + 14,
                      fill=body_color, outline=body_dark, width=2, tags="dyn")

        c.create_polygon(hx - 6, hy - 8,
                         hx - 14, hy + 4,
                         hx - 4, hy + 6,
                         fill=ear_color, outline=body_dark, width=1, tags="dyn")
        c.create_polygon(hx + 4, hy - 10,
                         hx - 2, hy + 4,
                         hx + 8, hy + 4,
                         fill=ear_color, outline=body_dark, width=1, tags="dyn")

        c.create_oval(hx + 4, hy + 4, hx + 18, hy + 16,
                      fill=belly, outline=body_dark, width=1, tags="dyn")

        c.create_oval(hx + 13, hy + 6, hx + 18, hy + 10,
                      fill="#1a1a1a", outline="", tags="dyn")

        c.create_arc(hx + 6, hy + 9, hx + 16, hy + 16,
                     start=200, extent=-140,
                     style="arc", outline=body_dark, width=1, tags="dyn")
        c.create_oval(hx + 9, hy + 13, hx + 13, hy + 17,
                      fill="#f48fb1", outline="", tags="dyn")
        ...
        c.create_oval(ex - 2, ey - 2, ex + 2, ey + 2,
                      fill="#1a1a1a", tags="dyn")
        c.create_oval(ex - 1, ey - 3, ex + 1, ey - 1,
                      fill="white", tags="dyn")
    ...
    def _draw_cat(self, c, cx, gy, t, color):
        if self.timer_paused:
            walk = 0.0; bob = 0.0; tail_sway = 0.0
        else:
            walk = t * 4.5
            bob  = math.sin(t * 9) * 1.8
            tail_sway = math.sin(t * 3) * 8

        cy = gy - 26 + bob
        body_color = "#9e9e9e"
        body_dark  = "#555555"
        belly      = "#e0e0e0"
        stripe     = self._darken(body_color, 0.3)

        tx_end = cx - 22
        ty_end = cy - 28 + tail_sway
        c.create_line(cx - 16, cy + 2,
                      cx - 20, cy - 8,
                      cx - 22, cy - 18,
                      tx_end, ty_end,
                      fill=body_color, width=6,
                      capstyle="round", smooth=True, tags="dyn")
        c.create_oval(tx_end - 4, ty_end - 4,
                      tx_end + 4, ty_end + 4,
                      fill=stripe, outline=body_dark, width=1, tags="dyn")
        ...
        for sx_ in [cx - 8, cx - 2, cx + 4, cx + 10]:
            c.create_line(sx_, cy - 6, sx_, cy - 2,
                          fill=stripe, width=2, tags="dyn")
        ...
        hx, hy = cx + 20, cy - 14
        c.create_oval(hx - 12, hy - 10, hx + 14, hy + 14,
                      fill=body_color, outline=body_dark, width=2, tags="dyn")

        c.create_polygon(hx - 10, hy - 6,
                         hx - 6, hy - 20,
                         hx - 2, hy - 8,
                         fill=body_color, outline=body_dark, width=1, tags="dyn")
        c.create_polygon(hx - 8, hy - 8,
                         hx - 6, hy - 16,
                         hx - 4, hy - 9,
                         fill=color, outline="", tags="dyn")
        c.create_polygon(hx + 2, hy - 8,
                         hx + 6, hy - 22,
                         hx + 12, hy - 6,
                         fill=body_color, outline=body_dark, width=1, tags="dyn")
        c.create_polygon(hx + 4, hy - 9,
                         hx + 6, hy - 18,
                         hx + 10, hy - 8,
                         fill=color, outline="", tags="dyn")

        c.create_line(hx - 4, hy - 8, hx - 2, hy - 4,
                      fill=stripe, width=2, tags="dyn")
        c.create_line(hx + 2, hy - 8, hx + 4, hy - 4,
                      fill=stripe, width=2, tags="dyn")

        c.create_oval(hx + 0, hy + 4, hx + 14, hy + 14,
                      fill=belly, outline=body_dark, width=1, tags="dyn")

        c.create_polygon(hx + 7, hy + 5,
                         hx + 10, hy + 5,
                         hx + 8, hy + 8,
                         fill="#f4a8b8", outline=body_dark, width=1, tags="dyn")

        c.create_line(hx + 8, hy + 8, hx + 8, hy + 11,
                      fill=body_dark, width=1, tags="dyn")
        c.create_arc(hx + 4, hy + 9, hx + 9, hy + 13,
                     start=0, extent=-180, style="arc",
                     outline=body_dark, width=1, tags="dyn")
        c.create_arc(hx + 8, hy + 9, hx + 13, hy + 13,
                     start=0, extent=-180, style="arc",
                     outline=body_dark, width=1, tags="dyn")

        for wy, wlen in [(hy + 7, 10), (hy + 10, 9)]:
            c.create_line(hx + 12, wy, hx + 12 + wlen, wy - 1,
                          fill="#cccccc", width=1, tags="dyn")
            c.create_line(hx + 2, wy, hx + 2 - wlen, wy - 1,
                          fill="#cccccc", width=1, tags="dyn")

        ex, ey = hx + 4, hy - 2
        c.create_oval(ex - 4, ey - 4, ex + 4, ey + 4,
                      fill="#c5e890", outline=body_dark, width=1, tags="dyn")
        c.create_oval(ex - 1, ey - 3, ex + 1, ey + 3,
                      fill="#1a1a1a", tags="dyn")
    ...
    def _darken_hex(self, color, factor=0.6):
        r, g, b = self._hex_to_rgb(color)
        return self._rgb_to_hex(r * (1 - factor), g * (1 - factor), b * (1 - factor))
